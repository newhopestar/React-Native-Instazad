...
const MoveBookmarkOptions = ({ route }: MoveBookmarkOptionsProps) => {
  ...
  const collections = useSelector((state) => state.user.bookmarks?.filter((x) => x.name !== "All Posts"));
    ...
    if (collections) {
      Promise.all(
        selectedIndexs.map(async (i) => {
          await dispatch(
            MoveBookmarkToCollectionRequest(fromCollectionName, targetCollectionName, bookmarks[i].postId)
          );
        })
      ).then(goBack);
    }
