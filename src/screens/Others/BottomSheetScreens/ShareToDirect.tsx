...
const ShareToDirect: FunctionComponent<Props> = ({ item }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const commonUsers = Array.from(new Set(currentMsgUsers.concat(followers).concat(followings)));
      ...
      const filteredUserInfoList = userInfoList.filter(
        (usr) =>
          ...
          (usr.privacySetting?.blockedAccounts?.blockedAccounts || []).indexOf(`${myUsername}`) < 0 &&
          (usr.privacySetting?.accountPrivacy?.private === false ||
            (usr.privacySetting?.accountPrivacy?.private === true && currentFollowings.indexOf(`${usr}`) > 0)) &&
          usr !== myUsername
      );
    ...
    <>
      <View style={styles.titleWrapper}>
        ...
        <View style={styles.messageInputWrapper}>
          <FastImage style={styles.previewImage} source={{ uri: item.source![0].uri }} />
          <TextInput
      ...
      <View style={styles.body}>
        ...
        <FlatList
          ...
          style={{
            height: SCREEN_HEIGHT * (isFullScreen ? 1 : 0.6) - 83.5 - 36 - 50,
            marginVertical: 5,
          }}
          data={receiverList.length === 0 ? suggestionList : receiverList}
          renderItem={({ item: user, index }) => (
            <ReceiverItem type={"post"} shareItem={item} message={message} index={index} user={user} />
          )}
...
const ReceiverItem = ({ user, index, shareItem, message, type }: ReceiverItemProps) => {
  ...
  const _onToggleSend = async () => {
    if (!sent) {
      const msg: PostingMessage = {
        ...
        seen: 0,
        postId: (shareItem as ExtraPost).uid,
        text: message,
      };
