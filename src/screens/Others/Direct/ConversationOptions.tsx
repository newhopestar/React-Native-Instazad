...
const ConversationOptions = ({ route }: ConversationOptionsProps) => {
  ...
  const fType =
    myFollowings.indexOf(targetUsername) > -1
      ? 1
      : conversation.ownUser.requestedList && conversation.ownUser.requestedList?.indexOf(myUsername) > -1
      ? 3
      : 2;
  ...
  //EFFECTS
  useEffect(() => {
    const currentMutedMessages = [...(store.getState().user.setting?.privacy?.mutedMessages?.mutedMessages || [])];
    if (mutedMessages) {
  ...
  useEffect(() => {
    const currentMutedVideoCalls = [
      ...(store.getState().user.setting?.privacy?.mutedVideoCalls?.mutedVideoCalls || []),
    ];
    if (mutedVideoChats) {
  ...
  return (
    <React.Fragment>
      {showConfirmBlock && (
        <TouchableOpacity onPress={() => setShowConfirmBlock(false)} activeOpacity={1} style={styles.backdrop}>
        ...
        <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.previewSharedImagesWrapper}>
            {previewSharedImages.map((msg, index) => (
              <SharedImage sharedImages={sharedImages} key={index} message={msg} index={index} />
            ))}
          </View>
          <View style={styles.userInfoWrapper}>
            ...
            >
              ...
              >
                ...
                >
                  {followType === 1 ? "Following" : followType === 3 ? "Requested" : "Follow"}
                </Text>
              </TouchableOpacity>
            </TouchableOpacity>
          </View>
          <View style={styles.extraOptionsWrapper}>
            ...
            <TouchableOpacity onPress={() => setShowConfirmBlock(true)} style={styles.extraOptionItem}>
              <Text
                style={{
                  fontSize: 16,
                }}
