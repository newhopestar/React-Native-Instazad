...
import { StoryImageSpec, StoryTakerRouteProp } from "./StoryTaker";
...
const BottomOptions: FunctionComponent<Props> = ({ route, useFront, onBigButtonPress }) => {
  const { sendToDirect, username } = route?.params || {};
  const focused = useIsFocused();
  const [front, toggleFront] = useFront();
  ...
  const ref = useRef<{ preGalleryOffsetY: number; showGroups: boolean }>({ preGalleryOffsetY: 0, showGroups: false });
  ...
  return (
    <>
      ...
      >
        ...
        <TouchableOpacity onPress={onBigButtonPress} activeOpacity={0.7} style={styles.btnTakePhoto}>
          <View
            style={{
              width: 70,
              height: 70,
        ...
        <TouchableOpacity activeOpacity={0.8} onPress={toggleFront}>
        ...
        >
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <TouchableOpacity onPress={_onSelectGroup.bind(null, index)} style={styles.btnGroup}>
                <Text
                  style={{
