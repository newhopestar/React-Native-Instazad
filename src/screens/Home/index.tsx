...
import LivePreviewList from "../../components/LivePreviewList/LivePreviewList";
...
const index = () => {
  ...
  const _onScroll = ({
    ...
    if (y / height > 0.45 && y > ref.current.preOffsetY && !loadingMore && !refreshing) {
      (async () => {
        setLoadingMore(true);
      ...
      <KeyboardAvoidingView style={styles.keyboardAvoidingViewContainer} behavior="height">
        ...
        {showCommentInput && (
          <CommentInputPopup
            ...
            preValue={
              ref.current.commentContents.filter((x) => x.id === ref.current.currentCommentId)[0]?.content || ""
            }
