        ...
        ListHeaderComponent={
          <>
            {lastRequest.hasOwnProperty("username") && (
              <TouchableOpacity onPress={() => navigate("ActivityFollowRequests")} style={styles.requestListWrapper}>
                <View style={{ height: 40, width: 40, marginRight: 10 }}>
                  <FastImage
                    source={{ uri: lastRequest.avatarURL }}
                    style={{ height: 40, width: 40, borderRadius: 40, borderColor: "#333", borderWidth: 0.3 }}
                  />
                  <View style={styles.requestNumber}>
                    <Text style={{ color: "#fff", fontWeight: "bold" }}>{extraInfo?.requestedList.length}</Text>
                  </View>
                </View>
                <View>
                  <Text>Follow Request</Text>
                  <Text style={{ color: "#666" }}>Approve or ignore requests</Text>
                </View>
              </TouchableOpacity>
            )}
          </>
        }
        renderItem={({ item, index }) => <NotificationItem {...{ item, key: index }} />}
        keyExtractor={(item, index) => `${index}`}
        ListFooterComponent={
          <View>
            <Text style={{ margin: 15, fontSize: 16, fontWeight: "500" }}>Suggestions For You</Text>
            ...
            <TouchableOpacity style={{ padding: 15 }} onPress={() => navigate("DiscoverPeople")}>
              <Text style={{ color: "#318bfb" }}>See All Suggestions</Text>
            </TouchableOpacity>
          </View>
        }
      />
      ...
      <View style={styles.requestBtnGroups}>
        ...
        >
          <Text
            style={{
              fontWeight: "600",
              color: item.followType === 1 || item.followType === 3 ? "#000" : "#fff",
            }}
          >
