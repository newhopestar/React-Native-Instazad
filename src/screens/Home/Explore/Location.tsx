...
const Location = ({ route }: LocationProps) => {
  ...
  const [addressInfo, setLocationInfo] = useState<MapBoxAddress>({
    ...address,
  });
  ...
  useEffect(() => {
    (async () => {
      const ref = firestore();
      const rq = await ref.collection("posts").where("address.id", "==", address.id).get();
      ...
      setLocationInfo({
        ...
        avatarURI: postList.length > 0 ? postList[0].source && postList[0].source[0].uri : "",
      });
  ...
  return (
    <>
      {showFullMapConfirm && (
        <TouchableOpacity onPress={setShowFullMapConfirm.bind(null, false)} activeOpacity={1} style={styles.backdrop}>
          <View style={styles.confirmWrapper}>
            ...
            <TouchableOpacity onPress={setShowFullMapConfirm.bind(null, false)} style={styles.btnConfirm}>
              <Text
                style={{
                  fontSize: 16,
                  fontWeight: "500",
      ...
      <SafeAreaView style={styles.container}>
        <View>
          ...
          <TouchableOpacity onPress={() => {}} style={styles.btnFeedback}>
            <Image
              style={{
                height: 20,
                width: 20,
        ...
        >
          ...
          <View style={styles.addressTabWrapper}>
            <TouchableOpacity onPress={_onSwitchTab.bind(null, 1)} style={styles.addressTab}>
              <Text
                style={{
                  fontWeight: "500",
            ...
            <TouchableOpacity onPress={_onSwitchTab.bind(null, 2)} style={styles.addressTab}>
              <Text
                style={{
                  fontWeight: "500",
        ...
        <ScrollView ref={_scrollRef} scrollEnabled={false} horizontal={true} showsHorizontalScrollIndicator={false}>
          <FlatList
            style={{
              flex: 1,
            }}
            ...
            renderItem={({ item, index }) => <PhotoItem {...{ item, index }} key={index} />}
            numColumns={3}
