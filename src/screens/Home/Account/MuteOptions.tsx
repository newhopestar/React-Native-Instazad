...
const MuteOptions = ({ route }: MuteOptionsProps) => {
  ...
  const [mutePost, setMutePost] = useState<boolean>(mutedPostAccounts.indexOf(targetUser.username || "") > -1);
  const [muteStories, setMuteStories] = useState<boolean>(mutedStoryAccounts.indexOf(targetUser.username || "") > -1);
