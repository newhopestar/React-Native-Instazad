...
const Follow = ({ route }: FollowProps) => {
  ...
  const [recentFollowerInteractionList, setRecentFollowerInteractionList] = useState<UserInfo[]>([]);
  const [recentFollowingsInteractionList, setRecentFollowingsInteractionList] = useState<UserInfo[]>([]);
  ...
  const _tabLineOffsetX = React.useMemo(() => new Animated.Value(((type - 1) * SCREEN_WIDTH) / 2), []);
  const ref = useRef({
    currentTab: type,
    ...
    if (followerQuery.length > 0) {
      ref.current.followerQueryTimeout = setTimeout(() => {
        const temp = [...followers].filter(
          (usr) => usr.username && usr.username.indexOf(followerQuery.toLowerCase()) > -1
        );
    ...
    if (followingQuery.length > 0) {
      ref.current.followingQueryTimeout = setTimeout(() => {
        const temp = [...follwings].filter(
          (usr) => usr.username && usr.username.indexOf(followingQuery.toLowerCase()) > -1
        );
  ...
  useEffect(() => {
    if (extraInfo) {
      ...
      const dontFollowUsrnames = [...followerUsrnames.filter((usr) => follwingUsrnames.indexOf(usr) < 0)];
      ...
      Promise.all(taskFollowers).then((result) => {
        ...
        const dontFollowBackUsers = [...result.filter((usr) => dontFollowUsrnames.indexOf(usr.username || "") > -1)];
        ...
        ref
          ...
          .then((rs) => {
            ...
            const temp = [...result.filter((usr) => interactionList.indexOf(usr.username || "") > -1)];
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {selectedIndex > -1 && (
        <TouchableOpacity activeOpacity={1} onPress={setSelectedIndex.bind(null, -1)} style={styles.confirmWrapper}>
          <View style={styles.confirmBox}>
            ...
            <TouchableOpacity onPress={setSelectedIndex.bind(null, -1)} style={styles.btnConfirm}>
              <Text
                style={{
                  fontSize: 16,
                }}
      ...
      {selectedUnfollowIndex > -1 && (
        ...
        >
          <View style={styles.confirmBox}>
            ...
            <TouchableOpacity onPress={setSelectedUnfollowIndex.bind(null, -1)} style={styles.btnConfirm}>
              <Text
                style={{
                  fontSize: 16,
                }}
            ...
            ListHeaderComponent={
              <>
                {lastRequest.hasOwnProperty("username") && (
                  <TouchableOpacity onPress={() => navigate("FollowRequests")} style={styles.requestListWrapper}>
                    <View
                      style={{
                        height: 40,
                        width: 40,
                    ...
                    {dontFollowBackList.length > 0 && (
                      ...
                      >
                        ...
                        <View>
                          ...
                          >
                            {dontFollowBackList[0].username} and {dontFollowBackList.length - 1} others
                          </Text>
                        </View>
                      </TouchableOpacity>
                    )}
                    {recentFollowerInteractionList.length > 0 && (
                      ...
                      >
                        ...
                        <View>
                          ...
                          >
                            {recentFollowerInteractionList[0].username} and {recentFollowerInteractionList.length - 1}{" "}
                            others
                          </Text>
                        </View>
                      </TouchableOpacity>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      {item.followType === 1 ? "Following" : item.followType === 2 ? "Follow" : "Requested"}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
