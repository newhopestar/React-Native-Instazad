...
const index = (): JSX.Element => {
  ...
  const [showingTurnOffNotificationModal, setShowingTurnOffNotificationModal] = useState<boolean>(false);
  const [switchOn, setSwitchOn] = useState<boolean>(notificationSetting?.pauseAll?.active || false);
  const route = useRoute();
  const currNavigation = settingNavigationMap.filter((x) => x.navigationName === route.name)[0];
  ...
  return (
    <>
      {showingTurnOffNotificationModal && (
        <TouchableOpacity onPress={_onTurnOffHandler} style={styles.pauseAllModal}>
          <View style={styles.pausAllOptionsWrapper}>
            ...
            <View style={{ width: "100%" }}>
              ...
              <TouchableOpacity onPress={_onTurnOnNotification.bind(null, 1 * 3600)} style={styles.pauseAllOptionItem}>
                <Text style={{ fontSize: 16, fontWeight: "500" }}>1 hour</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={_onTurnOnNotification.bind(null, 2 * 3600)} style={styles.pauseAllOptionItem}>
                <Text style={{ fontSize: 16, fontWeight: "500" }}>2 hours</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={_onTurnOnNotification.bind(null, 4 * 3600)} style={styles.pauseAllOptionItem}>
                <Text style={{ fontSize: 16, fontWeight: "500" }}>4 hours</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={_onTurnOnNotification.bind(null, 8 * 3600)} style={styles.pauseAllOptionItem}>
      ...
      >
        <NavigationBar
          title={settingNavigationMap.filter((x) => x.navigationName === route.name)[0].name}
          callback={() => {
            navigation.goBack();
          }}
        />
        ...
        <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
          ...
          >
            ...
            <Switcher on={switchOn} onTurnOff={_onTurnOffHandler} onTurnOn={_onTurnOnHandler} />
          </TouchableOpacity>
          {currNavigation.child &&
            currNavigation.child.map((settingNavigation, index) => (
              ...
              >
                ...
                >
                  {settingNavigation.icon && <Icon name={settingNavigation.icon} size={24} />}
                  <Text
                    style={{
                      fontSize: 16,
                      fontWeight: "400",
