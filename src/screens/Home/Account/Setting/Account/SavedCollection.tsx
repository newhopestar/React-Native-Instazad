...
const SavedCollection = ({ route }: SavedCollectionProps) => {
  ...
  const collection = useSelector((state) => state.user.bookmarks?.find((x) => x.name === collectionName));
  ...
  const _onRemove = () => {
    if (selectedIndexs.length === bookmarks.length) {
      ...
      selectedIndexs.map((index) => {
        dispatch(RemoveFromBookmarkCollectionRequest(bookmarks[index].postId, collectionName));
      });
      setSelectedIndexs([]);
    }
  };
  ...
  return (
    <React.Fragment>
      {showOptions && (
        <TouchableOpacity activeOpacity={1} onPress={() => setShowOptions(false)} style={styles.backdrop}>
      ...
      >
        <View style={styles.navigationBar}>
          ...
          <TouchableOpacity onPress={() => setShowOptions(true)} style={styles.btnNavigation}>
        ...
        {selectionMode && (
          <View style={styles.bottomOptions}>
            <TouchableOpacity onPress={_onRemove} disabled={selectedIndexs.length === 0} style={styles.bottomOption}>
              <Text
                style={{
                  fontSize: 16,
