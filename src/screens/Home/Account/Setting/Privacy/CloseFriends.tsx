...
const CloseFriends = (): JSX.Element => {
  ...
  const [currNavigation, setCurrNavigation] = useState<SettingNavigation | { name: string }>({ name: "" });
  useEffect(() => {
  ...
  const ref = useRef({ timeout: setTimeout(() => {}, 0) });
  useEffect(() => {
  ...
  const _onRefresh = () => {
    setCloseList(store.getState().user.setting?.privacy?.closeFriends?.closeFriends || []);
  ...
  const fetchSuggestionCloseFriends = async () => {
    const ref = firestore();
    const rq = await ref.collection("posts").where("userId", "==", myUsername).limit(25).get();
    let preList: {
      username: string;
      score: number;
    }[] = [];
    rq.docs.map(async (rs, index) => {
      ...
      rq2.docs.map((rs2) => {
        ...
        if (comment.userId !== myUsername && closeList.indexOf(comment.userId || "") < 0) {
          let i = -1;
          preList.every((x, index2) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {!searching && (
        <TouchableOpacity onPress={_onStartSearching} activeOpacity={1} style={styles.mockInputSearch}>
          <View
            style={{
              width: 40,
              height: 40,
      ...
      <ScrollView
        style={{
          height: SCREEN_HEIGHT - getTabBarHeight() - STATUS_BAR_HEIGHT - 44 - 90 - 70,
        }}
        ...
        {!searching && (
          <View style={styles.closeFriends}>
            ...
            {closeUsers.map((userX, index) => (
              <UserItem userX={userX} key={index} _onToggleCloseFriend={_onToggleCloseFriend} closeList={closeList} />
            ))}
          </View>
        )}
      ...
      >
        ...
        >
          {closeList.indexOf(userX.username || "") > -1 ? (
            <Image style={styles.star} source={require("../../../../../assets/icons/close-friend.png")} />
          ) : null}
        </ImageBackground>
        <View
          style={{
