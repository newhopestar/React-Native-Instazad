...
const MessageItem = ({ item, index, owner, showMsgEmojiSelection }: MessageItemProps) => {
  ...
  const _showEmojiOptions = ({ nativeEvent: { pageX, pageY } }: GestureResponderEvent) => {
  ...
  return (
    ...
    >
      ...
      >
        {item.type === messagesTypes.TEXT && <Text style={styles.msgText}>{item.text}</Text>}
        ...
        {item.type === messagesTypes.ADDRESS && <LocationMessage address_id={item.address_id as string} />}
        {item.type === messagesTypes.POST && (
          <PostMessage maxWidth={SCREEN_WIDTH * 0.6} postId={item.postId as number} text={item.text} />
        )}
