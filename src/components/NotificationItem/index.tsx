...
const NotificationItem = ({ item }: NotificationItemProps) => {
  ...
  const _onViewNotification = () => {
    if (item.type === notificationTypes.FOLLOW_ME) {
      navigate("ActiviyFollow", { type: 1 });
      return;
    }

    navigate("PostDetail", { postId: item.postId, postData: item.postInfo });
  };
          ...
          {item.previewFroms && (
            <>
              {item.previewFroms.length > 1 && (
                <>
                  <FastImage style={styles.avatar} source={{ uri: item.previewFroms[1].avatarURL }} />
                  <FastImage
                    style={{
                      ...styles.avatar,
                      transform: [
