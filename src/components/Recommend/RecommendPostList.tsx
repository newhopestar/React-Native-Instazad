  ...
  const fetchRecommendPosts = () => {
    return new Promise<RecommendPost[]>(async (resolve, reject) => {
      ...
      const tasks: Promise<RecommendPost[]>[] = enjoyLabels.map(async (label) => {
        const post = await postRef.orderBy("create_at", "desc").where("labels", "array-contains", label).get();
    ...
    <View style={[styles.container, containerStyle]}>
      {!loading && (
        <FlatList
          ...
          numColumns={3}
          data={recommendPosts}
          renderItem={({ item, index }) => <RecommendItem {...{ item, index, showClassMask: true }} />}
...
export async function getRecommendLabels(
  diffMonth: number
): Promise<{
