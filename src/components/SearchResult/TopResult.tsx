      ...
      {!searching && (
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <ResultItem showRemoveBtn={true} searchItem={history[index]} item={item} key={index} />
          )}
...
export const ResultItem = React.memo(({ showRemoveBtn, searchItem, item }: ResultItemProps) => {
  const dispatch = useDispatch();
  ...
  return (
    <TouchableOpacity onPress={() => _onViewResultDetail(item)} style={styles.userItem}>
      <View
        style={{
          flexDirection: "row",
          alignItems: "center",
