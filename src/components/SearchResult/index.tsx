...
const SearchResult = ({ query }: SearchResultProps) => {
  ...
  const history = useSelector((state) => state.user.user.userInfo?.searchRecent) || [];
        ...
        result = result.map((item) => {
          if ("username" in item) {
            if (
              ...
              ((item as MixedProfileX).requestedList || []).indexOf(myUsername) > -1
            ) {
  ...
  useEffect(() => {
    ...
    if (query.length > 0) {
      ref.current.timeout = setTimeout(async () => {
        const ref = firestore();
        const hashtag = await ref.collection("hashtags").where("keyword", "array-contains", query.trim()).get();
        const hashTagList: HashTag[] = hashtag.docs.map((doc) => doc.data() || {});
        const accounts = await ref.collection("users").where("keyword", "array-contains", query).get();
        const accountList: MixedProfileX[] = accounts.docs.map((x) => x.data() || {});
        const posts = await ref.collection("posts").where("address.keyword", "array-contains", query).get();
...
const SearchTab = ({ resultData, recentList, searching }: SearchTabProps) => {
  const filter = (list: (MixedProfileX | HashTag | MapBoxAddress)[], field: string[]) => [
  ...
  return (
    <Tab.Navigator>
      <Tab.Screen children={() => <TopResult {...{ resultData, recentList, searching }} />} name="Top" />
      <Tab.Screen
        children={() => (
          <Accounts searching={searching} recentList={filteredAccountRecents} resultData={filteredAccountResults} />
        )}
        name="Accounts"
      />
      <Tab.Screen
        children={() => (
          <Tags searching={searching} recentList={filteredHashtagRecents} resultData={filteredHashtagResults} />
        )}
        name="Tags"
      />
      <Tab.Screen
        children={() => (
          <Places searching={searching} recentList={filteredPlaceRecents} resultData={filteredPlaceResults} />
        )}
        name="Places"
      />
    </Tab.Navigator>
