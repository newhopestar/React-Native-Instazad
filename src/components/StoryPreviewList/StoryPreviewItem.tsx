...
const StoryPreviewItem = ({ item: { ownUser, storyList }, index }: StoryPreviewItemProps) => {
  ...
  useEffect(() => {
    const isSeen: boolean = storyList.every((story) => (story.seenList || []).indexOf(myUsername) > -1);
    setSeen(isSeen);
  }, [storyList]);
    ...
    <View style={styles.container}>
      <View style={styles.itemWrapper}>
        ...
        <View style={styles.imageContainer}>
          <TouchableOpacity onPress={_onShowStory} activeOpacity={0.8} style={styles.imageWrapper}>
            <FastImage style={styles.image} source={{ uri: ownUser?.avatarURL }} />
          </TouchableOpacity>
        </View>
      </View>
