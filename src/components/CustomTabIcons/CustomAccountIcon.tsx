...
const CustomAccountIcon = ({ focused }: CustomAccountIconProps) => {
  ...
  const collectionAll = useSelector((state) => state.user.bookmarks?.find((x) => x.name === "All Posts")) || {
    bookmarks: [],
  };
  ...
  const ref = useRef<{
    ...
    preBoormarkCount: store.getState().user.bookmarks?.find((x) => x.name === "All Posts")?.bookmarks.length || 0,
    animating: false,
  });
  useEffect(() => {
