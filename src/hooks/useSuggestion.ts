...
export function useSuggestion(
  limit?: number
): [ExtraSuggestionUserInfo[], React.Dispatch<React.SetStateAction<ExtraSuggestionUserInfo[]>>] {
  ...
  const fetchSuggestion = async () => {
    ...
    if (limit) recentInteractionsForTask = recentInteractionsForTask.splice(0, limit - recentInteractions.length);
    ...
    const recentInteractionTasks: Promise<ExtraSuggestionUserInfo>[] = recentInteractionsForTask.map((usr) =>
      asyncFetchUser(usr, 1)
    );
    ...
    if (limit)
      dontFolloweUsernameForTask = dontFolloweUsernameForTask.splice(0, limit - recentInteractionsForTask.length);
    const dontFollowBackTasks: Promise<ExtraSuggestionUserInfo>[] = dontFolloweUsernameForTask.map((usr) =>
      asyncFetchUser(usr, 2)
    );
    Promise.all(recentInteractionTasks.concat(dontFollowBackTasks)).then((result) => {
      let followedByInteractionList: string[] = [];
      result.map((usr) => {
        if (usr.type === 1) {
          followedByInteractionList = followedByInteractionList.concat((usr.followings || []).splice(0, 5));
        }
      });
      followedByInteractionList = Array.from(new Set(followedByInteractionList)).filter(
        (usr) =>
          usr !== myUsername &&
          follwingUsrnames.indexOf(usr) < 0 &&
          recentInteractionsForTask.indexOf(usr) < 0 &&
      ...
      if (limit)
        followedByInteractionForTask = followedByInteractionForTask.splice(
          0,
          limit - recentInteractionsForTask.length - dontFolloweUsernameForTask.length
        );
      const followedByInteractionTasks: Promise<ExtraSuggestionUserInfo>[] = followedByInteractionForTask.map((usr) =>
        asyncFetchUser(usr, 4)
      );

      Promise.all(followedByInteractionTasks).then((result2) => {
        const followerOfFollowingUsernameTasks: Promise<string>[] = follwingUsrnames.map(async (userX) => {
          const rq = await ref.collection("users").doc(userX).get();
          const userData: UserInfo = rq.data() || {};
          const userFollowings = userData.followings || [];
          if (userFollowings.length > 0) {
            let randomPickusername = userFollowings[Math.floor(Math.random() * userFollowings.length)];
        ...
        Promise.all(followerOfFollowingUsernameTasks).then((resultTemp) => {
          ...
          const followerOfFollowingsTasks: Promise<ExtraSuggestionUserInfo>[] = followedByFollwers.map((usr) =>
            asyncFetchUser(usr, 3)
          );
