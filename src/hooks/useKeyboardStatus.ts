...
export function useKeyboardStatus() {
  ...
  useEffect(() => {
    ...
    return () => {
      if (ref.current.keyboardShowListener) ref.current.keyboardShowListener.remove();
      if (ref.current.keyboardHideListener) ref.current.keyboardHideListener.remove();
    };
  });
