...
export const FetchNotificationListRequest = (): ThunkAction<Promise<void>, {}, {}, NotificationAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, NotificationAction>) => {
    try {
      ...
      const rq = await ref
        ...
        .get();
      const notificationTasks: Promise<ExtraNotification>[] = rq.docs.map(async (doc) => {
        const notification: ExtraNotification = doc.data() || {};
        const fromUsernames = [...(notification.froms || [])];
        fromUsernames.reverse();
        const previewFromTasks: Promise<UserInfo>[] = fromUsernames.splice(0, 2).map(async (usr) => {
        ...
        const post = await ref.collection("posts").doc(`${notification.postId}`).get();
        ...
        if (
          ...
          const rq2 = await ref.collectionGroup("comments").where("uid", "==", notification.commentId).get();
        ...
        if (
          ...
          const reply: Comment = comment.replies.find((reply: Comment) => reply.uid === notification.replyId) || {};
  ...
  return async (dispatch: ThunkDispatch<{}, {}, NotificationAction>) => {
    try {
      ...
      if (
        ...
        query = query.where("commentId", "==", notification.commentId).where("replyId", "==", 0);
      }
      if (
        notification.type === notificationTypes.LIKE_MY_REPLY ||
        notification.type === notificationTypes.REPLY_MY_COMMENT
        ...
        if (rq.size > 0) {
          ...
          if (currentFroms.length === 0) {
            ...
            targetNotification.ref.update({
              ...
              froms: currentFroms,
              ...(index < 0 && !notification.isUndo ? { create_at: new Date() } : {}),
            });
          }
        } else {
