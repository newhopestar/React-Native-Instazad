...
export const FetchProfileXRequest = (username: string): ThunkAction<Promise<void>, {}, {}, userXAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userXAction>) => {
    try {
      ...
      if (rq.exists) {
        ...
        if (
          (data.privacySetting?.blockedAccounts?.blockedAccounts || []).indexOf(myUsername) > -1 ||
          myBlockList.indexOf(username) > -1
        )
          data.isBlock = true;
        else data.isBlock = false;
        const photos = await ref.collection("posts").where("userId", "==", username).orderBy("create_at", "desc").get();
        ...
        const followers = await ref.collection("users").where("followings", "array-contains", username).get();
        ...
        data.mutualFollowings = (data.followings || []).filter(
          (usr) => (myUserData.followings || []).indexOf(usr) > -1 && usr !== myUsername && usr !== username
        );
...
export const ResetProfileXRequest = (): ThunkAction<Promise<void>, {}, {}, userXAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userXAction>) => {
