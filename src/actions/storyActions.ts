...
export const FetchStoryListRequest = (): ThunkAction<Promise<void>, {}, {}, StoryAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, StoryAction>) => {
    try {
      const ref = firestore();
      const me = store.getState().user.user;
      const myUsername = `${me.userInfo?.username}`;
      const request = await ref.collection("users").doc(me.userInfo?.username).get();
      ...
      if (request.exists) {
        ...
        while (followingList.length > 0) {
          ...
          const temp: Story[] = result.docs.map((doc) => {
            ...
            if (ownIds.indexOf(`${data.userId}`) < 0) ownIds.push(`${data.userId}`);
            return data;
          });
        ...
        while (ownIds.length > 0) {
          const result = await ref.collection("users").where("username", "in", ownIds.splice(0, 10)).get();
          ownInfos = ownInfos.concat(result.docs.map((doc) => doc.data()));
        }
        const fullStory: StoryList = ownInfos.map((info) => {
          const collectStory: Story[] = collection.filter((story) => story.userId == info.username);
        ...
        fullStory.map((x) => {
          x.storyList.sort((a, b) => (a.create_at?.toMillis() || 0) - (b.create_at?.toMillis() || 0));
        });
...
export const PostStoryRequest = (images: Story[]): ThunkAction<Promise<string[]>, {}, {}, StoryAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, StoryAction>) => {
    try {
      const ref = firestore();
      const me = store.getState().user.user;
      ...
      if (rq.exists) {
        ...
        for (let img of images) {
          ...
          (img.hashtags || []).map(async (hashtag) => {
            const rq = await ref.collection("hashtags").where("name", "==", hashtag).get();
            if (rq.size > 0) {
              ...
              const fetchRelatedTags: Promise<string[]>[] = keyword.map(async (character) => {
                const rq = await ref.collection("hashtags").where("keyword", "array-contains", character).get();
...
export const DeleteStoryRequest = (storyId: number): ThunkAction<Promise<string[]>, {}, {}, StoryAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, StoryAction>) => {
    try {
      const ref = firestore();
      const me = store.getState().user.user;
