...
export const FetchPostListRequest = (): ThunkAction<Promise<void>, {}, {}, PostAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, PostAction>) => {
    try {
      const me = store.getState().user.user;
      const myUsername = `${store.getState().user.user.userInfo?.username}`;
      const currentBlockedList = store.getState().user.setting?.privacy?.blockedAccounts?.blockedAccounts || [];
      const userRef = firestore().collection("users");
      const blockMe = await userRef
        .where("privacySetting.blockedAccounts.blockedAccounts", "array-contains", myUsername)
        .get();
      const blockedMeList = blockMe.docs.map((x) => x.data().username);
      const request = await firestore().collection("users").doc(me.userInfo?.username).get();
      ...
      if (result) {
        ...
        let collection: Post[] = [];
        while (follwingList.length > 0 && collection.length < LIMIT_POSTS_PER_LOADING) {
          ...
          const temp = rs.docs
            .map((doc) => {
              ...
              let post = { ...doc.data() };
              const rqCmt = doc.ref.collection("comments").orderBy("create_at", "desc").get();
              rqCmt.then((rsx) => {
                post.comments = rsx.docs.map((docx) => docx.data());
              });
              return post as Post;
            })
            .filter((x) => currentBlockedList.indexOf(`${x.userId}`) < 0 && blockedMeList.indexOf(`${x.userId}`) < 0);
          collection = collection.concat(temp);
        }
        let ownInfos: UserInfo[] = [];
        while (userIds.length > 0) {
          const rs = await firestore().collection("users").where("username", "in", userIds.splice(0, 10)).get();
 ...
 */
export const LoadMorePostListRequest = (): ThunkAction<Promise<void>, {}, {}, PostAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, PostAction>) => {
    try {
      const me = store.getState().user.user;
      const myUsername = `${store.getState().user.user.userInfo?.username}`;
      ...
      if (result) {
        ...
        while (follwingList.length > 0 && collection.length < LIMIT_POSTS_PER_LOADING) {
          const rs = await firestore()
            ...
            .get();
          rs.docs.map((doc) => {
            if (loadedUids.indexOf(doc.data().uid) < 0 && collection.length < LIMIT_POSTS_PER_LOADING) {
              if (userIds.indexOf(doc.data().userId) < 0) userIds.push(doc.data().userId);
              let post = { ...doc.data() };
              doc.ref
                .collection("comments")
        ...
        while (userIds.length > 0) {
          ...
          const rs = await firestore().collection("users").where("username", "in", usernames).get();
          const temp = rs.docs.map((doc) => {
            return doc.data();
          });
          ownInfos = ownInfos.concat(temp);
  ...
  return async (dispatch: ThunkDispatch<{}, {}, PostAction>) => {
    try {
      ...
      if (rq.size > 0) {
        ...
        const rq2 = await targetPost.ref.collection("comments").orderBy("create_at", "desc").get();
        if (postData && setPost) {
          const post = { ...postData };
...
export const ToggleLikePostRequest = (
  ...
  return async (dispatch: ThunkDispatch<{}, {}, PostAction>) => {
    try {
      ...
      const rq = await ref.collection("posts").where("uid", "==", postId).get();
      if (rq.docs.length > 0) {
        const targetPost: Post = (rq.docs[0].data() as Post) || {};
        const index = (targetPost.likes || []).indexOf(me.userInfo?.username || "");
        if (index > -1) {
...
//CREATE POST ACTION
export const CreatePostRequest = (postData: Post): ThunkAction<Promise<void>, {}, {}, PostAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, PostAction>) => {
    try {
