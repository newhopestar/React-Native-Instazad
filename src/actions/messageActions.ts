...
export const TriggerMessageListenerRequest = (): ThunkAction<Promise<void>, {}, {}, MessageAction> => {
    ...
    try {
      ...
      dbRef.ref(`/messages/${myUsernamePath}/`).once("value", async (snap) => {
        ...
        userIds.map((userId) => {
          ...
          dbRef
            .ref(`/messages/${myUsernamePath}/${convertToFirebaseDatabasePathName(userId)}`)
            .on("child_changed", async (snap: any) => {
          ...
          dbRef
            .ref(`/messages/${convertToFirebaseDatabasePathName(userId)}/${myUsernamePath}`)
            .on("child_changed", async (snap: any) => {
              const child: Message = snap.val();
              const uid = child.uid;
              const realUsername = revertFirebaseDatabasePathName(userId);
        ...
        const fetchUserStatusTasks = userIds.map((userId, index) => {
          return new Promise<OnlineStatus>((resolve, reject) => {
            dbRef.ref(`/online/${convertToFirebaseDatabasePathName(userId)}`).once("value", (snap3) => {
        ...
        Promise.all(fetchMyMessagesTasks).then(async () => {
          ...
          const fetchUserInfoListTasks: Promise<ProfileX>[] = userIds.map(async (userId) => {
            let idx = -1;
...
export const CreateMessageRequest = (
  ...
  return async (dispatch: ThunkDispatch<{}, {}, MessageAction>) => {
    try {
      ...
      dbRef.ref(`/messages/${targetUsernamePath}/${myUsernamePath}/${uid}`).set(msg);
...
export const MakeSeenRequest = (
  ...
  return async (dispatch: ThunkDispatch<{}, {}, MessageAction>) => {
    try {
      ...
      const myUsernamePath = convertToFirebaseDatabasePathName(myUsername);
      const dbRef = database();
      dbRef.ref(`/messages/${myUsernamePath}/${targetUsernamePath}/${msgUid}`).update({
        seen: seenTypes.SEEN,
      });
...
export const CreateEmptyConversationRequest = (
  ...
  return async (dispatch: ThunkDispatch<{}, {}, MessageAction>) => {
    try {
      ...
      const rq = await ref.collection("users").doc(`${targetUsername}`).get();
      if (rq.exists) {
        dbRef.ref(`/messages/${targetUsernamePath}/${myUsernamePath}`).set("NULL");
        dbRef.ref(`/messages/${myUsernamePath}/${targetUsernamePath}`).set("NULL");
        const extraMsg = store.getState().messages.find((x) => x.ownUser.username === targetUsername);
        const targetUserData: ProfileX = rq.data() || {};
        dbRef.ref(`/online/${targetUsernamePath}`).once("value", (snap) => {
          const newExtraMsg: ExtraMessage = {
      ...
      if (extraMsgIndex > -1) {
        ...
        if (msgIndex > -1) {
          ...
          if (msg.userId === targetUsername) {
            dbRef.ref(`/messages/${myUsernamePath}/${targetUsernamePath}/${msgId}/yourEmoji`).set(emoji);
          } else if (msg.userId === myUsername) {
            dbRef.ref(`/messages/${targetUsernamePath}/${myUsernamePath}/${msgId}/ownEmoji`).set(emoji);
          }
        } else dispatch(TriggerMessageListenerFailure());
      } else dispatch(TriggerMessageListenerFailure());
    } catch (e) {
...
export const UndoMyLastMessageRequest = (targetUsername: string): ThunkAction<Promise<void>, {}, {}, MessageAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, MessageAction>) => {
    try {
      const targetUsernamePath = convertToFirebaseDatabasePathName(targetUsername);
      const myUsername = store.getState().user.user.userInfo?.username || "";
