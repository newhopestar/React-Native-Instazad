...
export const LoginRequest = (user: userLoginWithEmail): ThunkAction<Promise<void>, {}, {}, userAction> => {
...
export const LogoutRequest = (): ThunkAction<Promise<void>, {}, {}, userAction> => {
...
export const RegisterRequest = (userData: RegisterParams): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    return auth()
...
export const UnfollowRequest = (username: string): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      let me: UserInfo = { ...store.getState().user.user.userInfo };
      const ref = firestore();
      const rq = await ref.collection("users").where("username", "==", me.username).get();
      if (rq.size > 0) {
        const targetUser = rq.docs[0];
        const user: UserInfo = targetUser.data() || {};
        if (user.followings !== undefined && user.followings.indexOf(username) > -1) {
 ...
 */
export const FetchExtraInfoRequest = (): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      let me: UserInfo = { ...store.getState().user.user.userInfo };
      const ref = firestore();
      const rq = await ref.collection("posts").where("userId", "==", me.username).orderBy("create_at", "desc").get();
      ...
      const rq2 = await ref.collection("users").where("username", "==", me.username).limit(1).get();
      if (rq2.size > 0) {
        ...
        const rq3 = await ref.collection("users").where("followings", "array-contains", me.username).get();
        payload.extraInfo.followers = rq3.docs.map((x) => x.data().username);
        const rq5 = await ref
          .collection("stories")
          .where("userId", "==", me.username)
          .where("create_at", ">=", new Date(new Date().getTime() - 24 * 3600 * 1000))
...
export const FetchExtraInfoSuccess = (extraInfo: ExtraInfoPayload): SuccessAction<ExtraInfoPayload> => {
  return {
...
//update extra info
export const UpdateExtraInfoRequest = (data: ExtraInfoPayload): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
...
//
export const FollowContactsRequest = (phoneList: string[]): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      let me: UserInfo = { ...store.getState().user.user.userInfo };
      const ref = firestore();
      ...
      if (rq.size > 0) {
        ...
        phoneList.map(async (phone, index) => {
          const rq2 = await ref.collection("users").where("phone", "==", phone).get();
          ...
          if (index === phoneList.length - 1) {
            userList.map(async (username, index2) => {
              const user: UserInfo = targetUser.data() || {};
              if (user.followings !== undefined && user.followings.indexOf(username) < 0 && username !== me.username) {
...
export const FollowUserSuccess = (payload: UserInfo): SuccessAction<UserInfo> => {
  return {
...
//SEND FOLLOW REQUEST
export const ToggleSendFollowRequest = (username: string): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      let me: UserInfo = { ...store.getState().user.user.userInfo };
      const ref = firestore();
...
export const UpdateUserInfoRequest = (updateUserData: UserInfo): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      let me: UserInfo = { ...store.getState().user.user.userInfo };
      const ref = firestore();
      ...
      if (rq.size > 0) {
        ...
        const { email, avatarURL, bio, birthday, followings, fullname, gender, phone, username, website } = userInfo;
...
export const UpdateNotificationSettingSuccess = (payload: NotificationSetting): SuccessAction<NotificationSetting> => {
  return {
...
export const UpdatePrivacySettingSuccess = (payload: PrivacySetting): SuccessAction<PrivacySetting> => {
  return {
...
export const UploadAvatarRequest = (uri: string, extension: string): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
...
export const RemoveFollowerRequest = (username: string): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      const me = store.getState().user.user.userInfo;
      const ref = firestore();
...
export const FetchSettingRequest = (): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    const me = store.getState().user.user.userInfo;
...
export const ConfirmFollowRequest = (username: string): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    const me = store.getState().user.user.userInfo;
    const ref = firestore();
    const rq = await ref.collection("users").doc(me?.username).get();
...
export const DeclineFollowRequest = (username: string): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    const me = store.getState().user.user.userInfo;
    const ref = firestore();
    const rq = await ref.collection("users").doc(me?.username).get();
...
export const UnSuggestionRequest = (username: string): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    const me = store.getState().user.user.userInfo;
    const ref = firestore();
    const rq = await ref.collection("users").doc(me?.username).get();
...
export const FetchRecentSearchRequest = (): ThunkAction<Promise<void>, {}, {}, userAction> => {
...
export const PushRecentSearchRequest = (searchItem: SearchItem): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    const me = store.getState().user.user.userInfo;
    const ref = firestore();
    const rq = await ref.collection("users").doc(me?.username).get();
    if (rq.exists) {
      ...
      const check = temp.every((item, index) => {
        if (
          (item.username === searchItem.username && searchItem.type === 1 && item.type === 1) ||
          (item.hashtag === searchItem.hashtag && searchItem.type === 2 && item.type === 2) ||
          (item.address === searchItem.address && searchItem.type === 3 && item.type === 3)
        ) {
...
export const RemoveRecentSearchRequest = (searchItem: SearchItem): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    const me = store.getState().user.user.userInfo;
    const ref = firestore();
    const rq = await ref.collection("users").doc(me?.username).get();
        ...
        const newCollections = collections
          .map((collection, index) => {
            ...
            if (index2 > -1) {
              ...
              if (collection.avatarIndex === index2 && collection.bookmarks.length > 0) {
...
export const RemoveBookmarkCollectionRequest = (
  ...
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      const collections = [...(store.getState().user.bookmarks || [])].filter((x) => x.name !== collectionName);
      dispatch({
        type: userActionTypes.UPDATE_BOOKMARK_SUCCESS,
        payload: {
          bookmarks: collections,
...
export const MoveBookmarkToCollectionRequest = (
  ...
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      ...
      const fromBookmarkIndex = collections[fromCollectionIndex].bookmarks.findIndex((x) => x.postId === postId);
      if (fromBookmarkIndex > -1) {
        ...
        const bookmark = newFromCollection.bookmarks.splice(fromBookmarkIndex, 1)[0];
        if (newFromCollection.avatarIndex === fromBookmarkIndex && newFromCollection.bookmarks.length > 0) {
...
//Archive Actions
export const FetchArchiveRequest = (): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      const ref = firestore();
...
export const AddStoryArchiveRequest = (storyList: StoryArchive[]): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      const ref = firestore();
      const myUsername = `${store.getState().user.user.userInfo?.username}`;
      const storyArchiveList = [...(store.getState().user.archive?.stories || [])];
...
export const AddPostArchiveRequest = (postList: PostArchive[]): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      const ref = firestore();
      const myUsername = `${store.getState().user.user.userInfo?.username}`;
...
export const RemovePostArchiveRequest = (uid: number): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      const ref = firestore();
      const myUsername = `${store.getState().user.user.userInfo?.username}`;
...
//Highlight actions
export const FetchHighlightRequest = (): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      const ref = firestore();
      const myUsername = `${store.getState().user.user.userInfo?.username}`;
      const rq = await ref.collection("users").doc(myUsername).get();
      const userData: ProfileX = rq.data() as ProfileX;
      const highlights = (userData.highlights || []).filter((x) => x.stories.length > 0);
      dispatch({
...
export const RemoveHighlightRequest = (targetHighlightName: string): ThunkAction<Promise<void>, {}, {}, userAction> => {
  return async (dispatch: ThunkDispatch<{}, {}, userAction>) => {
    try {
      const ref = firestore();
      const myUsername = `${store.getState().user.user.userInfo?.username}`;
