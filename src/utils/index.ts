...
export const convertDateToTimeStampFireBase = (date: Date): FirebaseFirestoreTypes.Timestamp => {
  return new FirebaseFirestoreTypes.Timestamp(
...
export const findUsersByName = async (q: string) => {
  let users: UserInfo[] = [];
  const ref = firestore();
  const rq = await ref.collection("users").where("keyword", "array-contains", q).get();
  rq.docs.map((x) => {
...
export const uploadSuperImages = (
  ...
  return images.map(async (img, index) => {
    ...
    return {
      sourceId: uid,
      hashtags: Array.from(new Set(img.labels.filter((x) => x.type === "hashtag").map((x) => x.text))),
      mention: Array.from(new Set(img.labels.filter((x) => x.type === "people").map((x) => x.text.slice(1)))),
      address: Array.from(
        new Set(
          img.labels
